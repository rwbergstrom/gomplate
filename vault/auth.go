...
func (v *Vault) GetToken() (string, error) {
	if token, err := v.AppRoleLogin(); token != "" || err != nil {
		return token, err
	}
	if token, err := v.AppIDLogin(); token != "" || err != nil {
		return token, err
	}
	if token, err := v.GitHubLogin(); token != "" || err != nil {
		return token, err
	}
	if token, err := v.UserPassLogin(); token != "" || err != nil {
		return token, err
	}
	if token, err := v.TokenLogin(); token != "" || err != nil {
		return token, err
	}
	if token, err := v.EC2Login(); token != "" || err != nil {
		return token, err
	}
...
func (v *Vault) AppRoleLogin() (string, error) {
	...
	secret, err := v.client.Logical().Write(path, vars)
	if err != nil {
		return "", errors.Wrapf(err, "AppRole logon failed", err)
	}
	if secret == nil {
...
func (v *Vault) UserPassLogin() (string, error) {
	...
	secret, err := v.client.Logical().Write(path, vars)
	if err != nil {
		return "", errors.Wrapf(err, "UserPass logon failed", err)
	}
	if secret == nil {
...
func (v *Vault) EC2Login() (string, error) {
	...
	if secret == nil {
		return "", errors.Wrapf(err, "Empty response from AWS EC2 logon")
	}
